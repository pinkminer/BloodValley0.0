...
public class EndingLinesPerform : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.L) && textIndex<linesCount && !isScrolling && !isScrolled && isFaded) //按键可以换成其它
        {
...
/*            else
    ...
    {
        canvasGroup.alpha = Mathf.Lerp(canvasGroup.alpha, 1, fadingSpeed * Time.deltaTime);
